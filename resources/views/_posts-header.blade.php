    ...
    <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-8">
        ...
        <div class="relative lg:inline-flex items-center bg-gray-100 rounded-xl">
            <div x-data="{ show: false }">
                ...
                <div x-show="show" class="py-2 absolute bg-gray-100 mt-2 rounded-cl w-full z-50" style="display: none">
                    <a
                    href="/"
                    class="block text-left px-3 text-sm leading-6 hover:bg-indigo-400 focus:bg-blue-500 hover:text-white focus:text-white">
                    Todas
                    </a>
                    @foreach($categories as $category)
                    <a
                    href="/categories/{{ $category->slug }}"
                    class="block text-left px-3 text-sm leading-6 hover:bg-indigo-400 focus:bg-indigo-400 hover:text-white focus:text-white
                    {{ isset($currentCategory) && $currentCategory->is($category) ? 'bg-indigo-400 text-white' : '' }}">
